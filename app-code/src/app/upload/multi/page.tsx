...
export default function MultiUploadPage() {
  ...
  function handleFiles(files: FileList) {
    ...
    const initial: ItemResult[] = list.map(() => ({
      photoDataUrl: "", category: "", brand: "", size: "", color: "",
      condition: "", style: "", description: "", analyzing: true, error: "",
    }));
    setItems(initial);
    setPhase("review");
    setReviewIndex(0);
    ...
    list.forEach((file, i) => {
      ...
      reader.onload = async () => {
        ...
        setItems((prev) => {
          const next = [...prev];
          next[i] = { ...next[i], photoDataUrl: dataUrl, analyzing: false, ...result };
          return next;
        });
      };
  ...
  return (
    <div style={{ width: "100%", minHeight: "100dvh", background: "#f9f4e8", fontFamily: FONT }}>
      <HeaderBar onBack={() => setPhase("pick")} title={`Article ${reviewIndex + 1} / ${total}`} />

      {/* Indicateurs */}
      <div style={{ display: "flex", gap: 6, padding: "12px 24px 0", justifyContent: "center" }}>
        {items.map((_, i) => (
          <button key={i} onClick={() => setReviewIndex(i)} style={{ width: 28, height: 6, borderRadius: 3, border: "none", cursor: "pointer", background: i === reviewIndex ? "#3c2f22" : "#d9d0c4", padding: 0 }} />
        ))}
      </div>
...
function HeaderBar({ onBack, title }: { onBack: () => void; title: string }) {
  return (
    <div style={{ background: "#3c2f22", height: "calc(68px + max(env(safe-area-inset-top), 44px))", borderBottomLeftRadius: 30, borderBottomRightRadius: 30, display: "flex", alignItems: "flex-end", paddingTop: "max(env(safe-area-inset-top), 44px)", paddingBottom: 14, paddingLeft: 16, paddingRight: 16 }}>
